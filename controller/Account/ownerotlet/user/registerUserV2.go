...
func RegisterUserV2(db *gorm.DB, w http.ResponseWriter, r *http.Request) {
	...
	user.FotoKtp = util.UploadPhotoAws(r, "FotoKtp", KodeUser, "user", "ktp")
	user.SelfieKtp = util.UploadPhotoAws(r, "SelfieKtp", KodeUser, "user", "ktp")
	user.FotoKontrak = util.UploadPhotoAws(r, "FotoKontrak", KodeUser, "user", "kontrak")
	user.Email = Email
	user.Hp = Hp
	toko.KodeUser = KodeUser
	toko.NamaToko = NamaToko
	toko.FotoToko = util.UploadPhotoAws(r, "FotoToko", KodeUser, "user", "Toko")
	user.FotoToko = util.UploadPhotoAws(r, "FotoToko", KodeUser, "user", "Toko")
